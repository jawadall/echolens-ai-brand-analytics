...
const companyNav = [
  { name: 'Overview', href: '/company', icon: ChartBarIcon },
  { name: 'Team Members', href: '/company/members', icon: UsersIcon },
  { name: 'Usage & Billing', href: '/company/usage', icon: ChartPieIcon },
  { name: 'Settings', href: '/company/settings', icon: Cog6ToothIcon },
]
...
export default function CompanyAdminLayout() {
  ...
  const companyName = user?.company_info?.name || user?.company || 'My Business'
  const planName = user?.company_info?.plan || 'free'
        ...
        <div className="flex h-full flex-col">
          ...
          <div className="flex h-[72px] items-center justify-between px-5" style={{ borderBottom: '1px solid var(--border-primary)' }}>
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-sm font-display font-bold truncate max-w-[140px]" style={{ color: 'var(--text-primary)' }}>{companyName}</h1>
            ...
            {companyNav.map((item) => {
              ...
              return (
                <Link key={item.name} to={item.href} onClick={() => setSidebarOpen(false)}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 group text-sm ${isActive ? 'text-indigo-400' : ''}`}
                  style={{
                    background: isActive ? 'rgba(99, 102, 241, 0.1)' : 'transparent',
                    color: isActive ? undefined : 'var(--text-secondary)',
                    border: isActive ? '1px solid rgba(99, 102, 241, 0.15)' : '1px solid transparent',
                  }}>
                  <item.icon className={`w-[18px] h-[18px] ${isActive ? 'text-indigo-400' : 'group-hover:text-indigo-400'} transition-colors`} />
          ...
          <div className="p-3 space-y-2" style={{ borderTop: '1px solid var(--border-primary)' }}>
            ...
            <div className="flex items-center gap-2.5 p-2.5 rounded-xl" style={{ background: 'var(--bg-elevated)' }}>
              <div className="w-9 h-9 rounded-full flex items-center justify-center text-white font-semibold text-sm flex-shrink-0 bg-gradient-to-br from-indigo-500 to-purple-500">
                {user?.first_name?.[0]}{user?.last_name?.[0]}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium truncate" style={{ color: 'var(--text-primary)' }}>{user?.full_name}</p>
                <p className="text-[11px] truncate" style={{ color: 'var(--text-muted)' }}>Business Admin</p>
              </div>
              <button onClick={handleLogout} className="p-1.5 rounded-lg transition-colors hover:bg-indigo-500/10" style={{ color: 'var(--text-muted)' }} title="Logout">
                <ArrowRightOnRectangleIcon className="w-4 h-4 hover:text-indigo-400" />
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="flex h-14 items-center justify-between px-4 lg:px-6">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-sm font-display font-semibold" style={{ color: 'var(--text-primary)' }}>
                {companyNav.find(n => n.href === location.pathname)?.name || 'Business Admin'}
              </h2>
            </div>
